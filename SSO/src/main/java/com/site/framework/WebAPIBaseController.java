...
public abstract class WebAPIBaseController {
	...
	public @ResponseBody ResponseEntity<String> handleMethodArgumentNotValidException(HttpServletRequest req,
	...
	public @ResponseBody ResponseEntity<Map<String, Object>> handleServiceException(HttpServletRequest req,
			ServiceException e) {
		...
		if (e.getErrorCode().equals("") || e.getErrorCode() == null)
			...
			return new ResponseEntity<Map<String, Object>>(MercuriusBeanUtils.killNull(e.getErrorCode()), e.getMessage(), new HashMap<String, Object>());
	}

	@ExceptionHandler(SystemException.class)
	public @ResponseBody ResponseEntity<Map<String, Object>> handleSystemException(HttpServletRequest req,
			SystemException e) {
	...
	public static class ValidationErrorSimple {
		...
		public ValidationErrorSimple(ObjectError e) {
			...
			this.message = MessageFormat.format(MercuriusBeanUtils.killNull(e.getDefaultMessage()), e.getArguments());
			this.code = e.getCode();
